...
RC 蓄積の根本解決には、以下のいずれかが必要：

1. **`emit_deep_clone` の Struct を true deep copy に変更**: 新メモリ malloc + フィールド再帰 clone → source と dest が独立 → CLEANUP_FULL が安全
2. **SRET return のムーブセマンティクス厳格化**: source の ownership を完全に dest に移転し、source のフィールドへのアクセスを禁止

いずれも大規模な変更が必要であり、現在の exit(0) 戦略は安全なワークアラウンドとして有効。
