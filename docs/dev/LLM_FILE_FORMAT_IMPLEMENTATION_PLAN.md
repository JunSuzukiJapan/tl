...
### 不足機能1: 16進リテラル `0x...` (修正済み)

ビット演算 `^`(XOR) と `|`(OR) は実装済みだったが、16進リテラル (`0xFF` 等) が未実装だったため、組み合わせて使おうとするとパースエラーになっていた。

**修正**: レキサーに `0x` (16進)、`0b` (2進)、`0o` (8進) リテラルを追加。
