# TL ドキュメント（日本語）

## コンセプト

- [TensorLanguage (TL) における論理プログラミング](logic_programming.md)
- [テンソル内包表記ガイド](tensor_comprehension_jp.md)
- [テンソル内包表記で実現したかったこと](tensor_comprehension_design.md)
- [Tensor Logic: なぜ単なるテンソルよりも多くの問題を解けるのか](tensor_logic_advantages_jp.md)
- [モデル学習（トレーニング）ガイド](training_guide.md)

## リファレンス

- [API リファレンス](reference/functions_and_types.md)
- [言語仕様](reference/language_spec.md)
- [標準ライブラリ概要](reference/standard_library.md)
