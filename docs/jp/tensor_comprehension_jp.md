...
`<clauses>` で定義されているが、出力 `<indices>` に **含まれない** 変数は、**リダクション変数** として扱われます。これらの変数について総和（Sum）が計算されます。これは **暗黙的リダクション解析（Einstein summation convention）** として実装されており、ボディ式を解析してリダクション変数を自動検出します。
