...
impl Monomorphizer {
    ...
    fn scan_module(&mut self, module: &mut Module) {
        ...
        for mut impl_block in module.impls.drain(..) {
            ...
            if !impl_block.generics.is_empty() || is_generic_target {
                ...
                let subst = if let Type::Struct(name, args) | Type::Enum(name, args) = &impl_block.target_type {
                    self.build_subst_from_type(name, args)
                } else {
                    HashMap::new()
                };
        ...
        match ty {
            Type::Struct(name, args) | Type::Enum(name, args) => {
                 ...
                 if is_enum {
                     ...
                     // Check if base name is a known enum (e.g., Entry[i64][i64] -> Entry)
                     let base_name = mangle_base_name(name);
    ...
    fn rewrite_stmt(&mut self, stmt: &mut StmtKind, subst: &HashMap<String, Type>, expected_type: Option<&Type>) {
         ...
         match expr {
             ExprKind::StructInit(ty, fields) => {
                ...
                // Try to resolve generic struct instantiation             ExprKind::StructInit(name, fields) => {
                 ...
                 // 3. Try base name lookup for mangled names (e.g., "Vec[i64]" -> "Vec")
                 else if name.contains('[') {
                     let orig_base = mangle_base_name(name);
                     if let Some(def) = self.generic_structs.get(orig_base) {
                         // Use args from the Type (resolve_type preserves them)
                         let args = match ty {
                             Type::Struct(_, a) | Type::Enum(_, a) => a,
                             _ => &vec![],
                         };
                         if !args.is_empty() {
              ...
              ExprKind::EnumInit { enum_name, variant_name, generics, payload } => {
                   ...
                   if !generics.is_empty() {
                       ...
                       *enum_name = concrete_name;
                       // Generics preserved for codegen type info
                   }
                      ...
                      if let Pattern::EnumPattern { enum_name, variant_name: _, bindings: _ } = pattern {
                          ...
                          if let Some(angle_idx) = enum_name.find('<') {
                              ...
                              if !substituted_args.is_empty() {
                                  let new_name = mangle_wrap_args(&base, &substituted_args);
                                  *enum_name = new_name;
                              }
                          }
                      }
     ...
     fn instantiate_impls(&mut self, name: &str, args: &[Type]) {
         let concrete_struct_name = if let Some(mangled) = self.struct_instances.get(&(name.to_string(), args.to_vec())) {
             ...
             mangle_wrap_args(name, &self.mangle_types_vec(args))
         };
     ...
     fn request_struct_instantiation(&mut self, name: &str, args: Vec<Type>) -> String {
        ...
        let mangled = mangle_wrap_args(name, &self.mangle_types_vec(&args));
    ...
    fn mangle_types_vec(&self, types: &[Type]) -> Vec<String> {
        types.iter().map(|t| self.mangle_type(t)).collect()
    }
    ...
    fn mangle_type(&self, ty: &Type) -> String {
        match ty {
            ...
            Type::Tensor(inner, _) => {
                let args = vec![self.mangle_type(inner)];
            ...
            Type::Path(segments, args) => {
                ...
                if args.is_empty() {
                    ...
                    mangle_wrap_args(&base, &self.mangle_types_vec(args))
                }
            }
            Type::Tuple(types) => {
                if types.is_empty() {
    ...
    /// Build a substitution map from a type's name and args.
    /// Maps the generic parameters (K, V) from the original definition
    /// to the concrete type arguments.
    fn build_subst_from_type(&self, name: &str, args: &[Type]) -> HashMap<String, Type> {
        let mut subst = HashMap::new();
        if args.is_empty() {
            return subst;
        }
        ...
        // Try generic structs
        if let Some(def) = self.generic_structs.get(base) {
        ...
        // Try generic enums
        if let Some(def) = self.generic_enums.get(base) {
            if args.len() == def.generics.len() {
                for (param, arg) in def.generics.iter().zip(args.iter()) {
                    subst.insert(param.clone(), arg.clone());
                }
