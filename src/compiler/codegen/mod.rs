...
impl<'ctx> CodeGenerator<'ctx> {
    ...
    fn compile_struct_defs(&mut self, structs: &[StructDef]) -> Result<(), String> {
        // Pass 1: Opaque
        for s in structs {
            // eprintln!("DEBUG: compile_struct_defs Pass 1 seeing {}", s.name);
        ...
        // Pass 2: Body
        for s in structs {
            // eprintln!("DEBUG: compile_struct_defs Pass 2 setting body for {}", s.name);
    ...
    fn compile_impl_blocks(&mut self, impls: &[ImplBlock]) -> Result<(), String> {
        ...
        for imp in impls {
            ...
            // eprintln!("DEBUG: compile_impl_blocks checking {} generics_len={}", target_name, imp.generics.len());
        ...
        for imp in impls {
            ...
            for method in &imp.methods {
                ...
                // SRET is disabled; keep signatures simple.
                let uses_sret = matches!(method.return_type, Type::Struct(_, _));
                // eprintln!("DEBUG: method {} uses_sret={}", mangled_name, uses_sret);
                let param_offset = if uses_sret { 
                     // Register SRET pointer
                     if let Some(sret_param) = function.get_nth_param(0) {
                         // eprintln!("DEBUG: found sret param for {}", mangled_name);
                         if sret_param.is_pointer_value() {
                             sret_param.set_name("sret_ptr");
                             self.current_sret_dest = Some(sret_param.into_pointer_value());
                             // eprintln!("DEBUG: set current_sret_dest for {}", mangled_name);
                         } else {
                             // eprintln!("DEBUG: sret param is NOT pointer for {}", mangled_name);
                         }
                     } else {
                         // eprintln!("DEBUG: NO sret param found for {}, count={}", mangled_name, function.count_params());
                     }
                     1 
                } else { 
                            ...
                            match &ty {
                                ...
                                Type::Struct(_, _) => {
                                    ...
                                    if let Some(sret_ptr) = self.current_sret_dest {
                                         // eprintln!("DEBUG: Implicit Return uses_sret=true");
    ...
    pub(crate) fn compile_fn_proto(&mut self, func: &FunctionDef) -> Result<FunctionValue<'ctx>, String> {
        ...
        // eprintln!("DEBUG: compile_fn_proto added function: {} type: {:?}", func.name, fn_type);
