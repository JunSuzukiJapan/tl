...
impl GpuOps for MetalTensor {
    ...
    fn cat(tensors: &[&Self], axis: usize) -> Self { MetalTensor::cat_impl(tensors, axis) }
    ...
    fn where_cond(condition: &Self, x: &Self, y: &Self) -> Self { MetalTensor::where_cond_impl(condition, x, y) }
