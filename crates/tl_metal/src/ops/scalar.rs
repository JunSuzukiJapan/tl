...
impl MetalTensor {
    ...
    pub fn pow_scalar_impl(&self, exp: f32) -> MetalTensor {
        self.scalar_op_1(SHADER_POW_SCALAR_F32, exp)
    }

    pub fn fmod_scalar_impl(&self, s: f32) -> MetalTensor {
        self.scalar_op_1(SHADER_FMOD_SCALAR_F32, s)
    }

    /// 1 パラメータスカラー演算共通ヘルパー
    fn scalar_op_1(&self, shader_name: &str, scalar: f32) -> MetalTensor {
        let result = MetalTensor::uninit(MetalTensor::shape(self), MetalTensor::dtype(self));
        let device = get_device();
        let command_queue = device.command_queue();
