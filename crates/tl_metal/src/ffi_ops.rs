...
pub fn tl_metal_batch_norm(
    ...
    let w_default_res = if w.is_none() {
        match MetalTensor::ones(x.shape(), DType::F32) {
            Ok(t) => Some(t),
            Err(e) => { eprintln!("batch_norm: ones for weight failed: {}", e); return std::ptr::null_mut(); }
        }
    } else { None };
    ...
    let var_default_res = if var.is_none() {
        match MetalTensor::ones(x.shape(), DType::F32) {
            Ok(t) => Some(t),
            Err(e) => { eprintln!("batch_norm: ones for var failed: {}", e); return std::ptr::null_mut(); }
        }
    } else { None };
...
pub fn tl_metal_layer_norm(
    ...
    let w_default = if w.is_none() {
        match MetalTensor::ones(x.shape(), DType::F32) {
            Ok(t) => Some(t),
            Err(e) => { eprintln!("layer_norm: ones for weight failed: {}", e); return std::ptr::null_mut(); }
        }
    } else { None };
...
#[no_mangle]
pub fn tl_metal_linspace(start: f64, end: f64, steps: usize) -> *mut OpaqueTensor {
    if steps == 0 {
        return make_tensor(MetalTensor::from_slice(&[] as &[f32], &[0], DType::F32));
    }
    if steps == 1 {
        return make_tensor(MetalTensor::from_slice(&[start as f32], &[1], DType::F32));
    }
    let step = (end - start) / (steps - 1) as f64;
    let data: Vec<f32> = (0..steps).map(|i| (start + step * i as f64) as f32).collect();
    make_tensor(MetalTensor::from_slice(&data, &[steps], DType::F32))
}

#[no_mangle]
pub fn tl_metal_rand(rank: i64, shape_ptr: *const usize, req_grad: bool) -> *mut OpaqueTensor {
