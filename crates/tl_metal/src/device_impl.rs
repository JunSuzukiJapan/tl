...
use tl_backend::{IDevice, BackendResult, BackendError, GpuTensor};
use crate::ffi_ops;
use crate::ffi;
...
fn v(p: *mut MetalTensor) -> BackendResult<*mut c_void> {
    if p.is_null() {
        Err(BackendError::InternalError("Metal Backend FFI returned null pointer (operation failed)".into()))
    } else {
        Ok(p as *mut c_void)
    }
}
...
impl IDevice for MetalDeviceImpl {
    // ========== テンソル作成 ==========
    #[inline] fn tensor_new(&self, data: *const f32, rank: usize, shape: *const usize) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_new(data, rank, shape)) }
    #[inline] fn tensor_new_i64(&self, data: *const i64, rank: usize, shape: *const usize) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_new_i64(data, rank, shape)) }
    #[inline] fn tensor_from_i64_array(&self, data: *const i64, len: i64) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_from_i64_array(data, len)) }
    #[inline] fn tensor_zeros(&self, rank: usize, shape: *const usize, req_grad: bool) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_zeros(rank, shape, req_grad)) }
    #[inline] fn tensor_ones(&self, rank: usize, shape: *const usize, req_grad: bool) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_ones(rank, shape, req_grad)) }
    #[inline] fn tensor_randn_debug(&self, rank: usize, shape: *const usize, seed: u64, req_grad: bool) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_randn_debug(rank, shape, seed, req_grad)) }
    ...
    fn tensor_group_norm(&self, input: *mut c_void, num_groups: i64, weight: *mut c_void, bias: *mut c_void, eps: f64) -> BackendResult<*mut c_void> {
        ...
        for b in 0..batch {
            for g in 0..num_groups as usize {
                ...
                for c in start_c..end_c { for s in 0..spatial {
                    let idx = b*channels*spatial + c*spatial + s;
                    let mut v = (result[idx] - mean) / std;
                    if !weight.is_null() { v *= unsafe { &*t(weight) }.to_vec::<f32>()[c]; }
                    if !bias.is_null() { v += unsafe { &*t(bias) }.to_vec::<f32>()[c]; }
                    result[idx] = v;
                }}
            }
        }
        Ok(ffi_ops::make_tensor(MetalTensor::from_slice(&result, &shape, crate::DType::F32)) as *mut c_void)
    }
    ...
    fn tensor_adaptive_avg_pool2d(&self, input: *mut c_void, output_h: i64, output_w: i64) -> BackendResult<*mut c_void> {
        ...
        for bi in 0..n { for ci in 0..c { for oi in 0..oh { for oj in 0..ow {
            ...
            for hi in hs..he { for wi in ws..we { sum += data[bi*c*h*w+ci*h*w+hi*w+wi]; cnt+=1; }}
    ...
    fn tensor_pad(&self, input: *mut c_void, pad_left: i64, pad_right: i64, value: f32) -> BackendResult<*mut c_void> {
        ...
        for o in 0..outer {
            ...
            for i in 0..last { result.push(data[o*last+i]); }
            for _ in 0..pad_right { result.push(value); }
        }
        let mut new_shape = shape; *new_shape.last_mut().unwrap() = new_last;
        Ok(ffi_ops::make_tensor(MetalTensor::from_slice(&result, &new_shape, crate::DType::F32)) as *mut c_void)
    }
    ...
    fn tensor_dropout2d(&self, input: *mut c_void, _p: f64, _training: bool) -> BackendResult<*mut c_void> {
    ...
    fn tensor_top_k_sample(&self, logits: *mut c_void, k: i64) -> BackendResult<*mut c_void> {
        ...
        let data = tl.to_vec::<f32>(); let k = k as usize;
        let mut sorted = data.clone();
        sorted.sort_by(|a, b| b.partial_cmp(a).unwrap_or(std::cmp::Ordering::Equal));
        let thr = if k < sorted.len() { sorted[k-1] } else { sorted[sorted.len()-1] };
        let masked: Vec<f32> = data.iter().map(|&v| if v >= thr { v } else { f32::NEG_INFINITY }).collect();
    ...
    fn tensor_top_p_sample(&self, logits: *mut c_void, p: f64) -> BackendResult<*mut c_void> {
        ...
        let mut cum = 0.0f32; let mut mask = vec![false; probs.len()];
        for &i in &indices { cum += probs[i]; mask[i] = true; if cum >= p as f32 { break; } }
        ...
        let idx = fp.iter().enumerate().max_by(|a,b| a.1.partial_cmp(b.1).unwrap_or(std::cmp::Ordering::Equal)).map(|(i,_)| i).unwrap_or(0);
        Ok(ffi_ops::make_tensor(MetalTensor::from_slice(&[idx as f32], &[1], crate::DType::F32)) as *mut c_void)
    }
    ...
    #[inline] fn tensor_prepare_return(&self, a: *mut c_void) -> BackendResult<*mut c_void> { Ok(a) }

    // ========== テンソル情報 ==========
    #[inline] fn tensor_len(&self, a: *mut c_void) -> BackendResult<usize> { Ok(ffi_ops::tl_metal_len(t(a))) }
    #[inline] fn tensor_dim(&self, a: *mut c_void, dim: usize) -> BackendResult<usize> { Ok(ffi_ops::tl_metal_dim(t(a), dim)) }
    #[inline] fn tensor_numel(&self, a: *mut c_void) -> BackendResult<i64> { Ok(ffi::tl_metal_numel(t(a))) }
    #[inline] fn tensor_data(&self, a: *mut c_void) -> BackendResult<*const f32> { Ok(ffi::tl_metal_data(t(a)) as *const f32) }
    #[inline] fn tensor_device_id(&self, a: *mut c_void) -> BackendResult<i32> { Ok(ffi_ops::tl_metal_device_id(t(a))) }
    #[inline] fn tensor_get_shape(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_get_shape(t(a))) }

    // ========== 要素アクセス ==========
    #[inline] fn tensor_get(&self, a: *mut c_void, idx: i64) -> BackendResult<f32> { Ok(ffi_ops::tl_metal_get(t(a), idx)) }
    #[inline] fn tensor_get_f32_md(&self, a: *mut c_void, indices: *const i64, rank: i64) -> BackendResult<f32> {
    ...
    #[inline] fn tensor_get_i64_md(&self, a: *mut c_void, indices: *const i64, rank: i64) -> BackendResult<i64> {
        let slice = unsafe { std::slice::from_raw_parts(indices, rank as usize) };
        if slice.len() >= 2 {
    ...
    #[inline] fn tensor_set_f32_md(&self, a: *mut c_void, indices: *const i64, rank: usize, value: f32) -> BackendResult<*mut c_void> {
        v(ffi_ops::tl_metal_set_f32_md(t(a), indices, rank, value))
    }
    #[inline] fn tensor_item(&self, a: *mut c_void) -> BackendResult<f32> { Ok(ffi_ops::tl_metal_item(t(a))) }
    #[inline] fn tensor_item_i64(&self, a: *mut c_void) -> BackendResult<i64> { Ok(ffi_ops::tl_metal_item_i64(t(a))) }

    // ========== 二項演算 ==========
    #[inline] fn tensor_add(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_add(t(a), t(b))) }
    #[inline] fn tensor_sub(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_sub(t(a), t(b))) }
    #[inline] fn tensor_mul(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_mul(t(a), t(b))) }
    #[inline] fn tensor_div(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_div(t(a), t(b))) }
    #[inline] fn tensor_rem(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_rem(t(a), t(b))) }
    #[inline] fn tensor_matmul(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_matmul(t(a), t(b))) }
    #[inline] fn tensor_pow(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_pow(t(a), t(b))) }
    #[inline] fn tensor_cross_entropy(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_cross_entropy(t(a), t(b))) }

    // ========== 単項演算 ==========
    #[inline] fn tensor_neg(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_neg(t(a))) }
    #[inline] fn tensor_abs(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_abs(t(a))) }
    #[inline] fn tensor_contiguous(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_contiguous(t(a))) }

    // ========== 比較演算 ==========
    #[inline] fn tensor_eq(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_eq(t(a), t(b))) }
    #[inline] fn tensor_neq(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_neq(t(a), t(b))) }
    #[inline] fn tensor_gt(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_gt(t(a), t(b))) }
    #[inline] fn tensor_lt(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_lt(t(a), t(b))) }
    #[inline] fn tensor_ge(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_ge(t(a), t(b))) }
    #[inline] fn tensor_le(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_le(t(a), t(b))) }

    // ========== スカラー演算 ==========
    #[inline] fn tensor_add_scalar(&self, a: *mut c_void, s: f64) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_add_scalar(t(a), s)) }
    #[inline] fn tensor_sub_scalar(&self, a: *mut c_void, s: f64) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_sub_scalar(t(a), s)) }
    #[inline] fn tensor_mul_scalar(&self, a: *mut c_void, s: f64) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_mul_scalar(t(a), s)) }
    #[inline] fn tensor_div_scalar(&self, a: *mut c_void, s: f64) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_div_scalar(t(a), s)) }
    #[inline] fn tensor_pow_scalar(&self, a: *mut c_void, exp: f32) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_pow_scalar(t(a), exp as f64)) }
    #[inline] fn tensor_scale(&self, a: *mut c_void, s: f32) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_scale(t(a), s)) }

    // ========== インプレース演算 ==========
    #[inline] fn tensor_add_assign(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<()> { ffi_ops::tl_metal_add_assign(t(a), t(b)); Ok(()) }
    #[inline] fn tensor_sub_assign(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<()> { ffi_ops::tl_metal_sub_assign(t(a), t(b)); Ok(()) }
    #[inline] fn tensor_mul_assign(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<()> { ffi_ops::tl_metal_mul_assign(t(a), t(b)); Ok(()) }
    #[inline] fn tensor_div_assign(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<()> { ffi_ops::tl_metal_div_assign(t(a), t(b)); Ok(()) }
    #[inline] fn tensor_mod_assign(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<()> { ffi_ops::tl_metal_mod_assign(t(a), t(b)); Ok(()) }
    #[inline] fn tensor_add_assign_scalar_f32(&self, a: *mut c_void, s: f32) -> BackendResult<()> { ffi_ops::tl_metal_add_assign_scalar_f32(t(a), s); Ok(()) }
    #[inline] fn tensor_sub_assign_scalar_f32(&self, a: *mut c_void, s: f32) -> BackendResult<()> { ffi_ops::tl_metal_sub_assign_scalar_f32(t(a), s); Ok(()) }
    #[inline] fn tensor_mul_assign_scalar_f32(&self, a: *mut c_void, s: f32) -> BackendResult<()> { ffi_ops::tl_metal_mul_assign_scalar_f32(t(a), s); Ok(()) }
    #[inline] fn tensor_div_assign_scalar_f32(&self, a: *mut c_void, s: f32) -> BackendResult<()> { ffi_ops::tl_metal_div_assign_scalar_f32(t(a), s); Ok(()) }
    #[inline] fn tensor_mod_assign_scalar_f32(&self, a: *mut c_void, s: f32) -> BackendResult<()> { ffi_ops::tl_metal_mod_assign_scalar_f32(t(a), s); Ok(()) }

    // ========== 数学・活性化関数 ==========
    #[inline] fn tensor_exp(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_exp(t(a))) }
    #[inline] fn tensor_log(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_log(t(a))) }
    #[inline] fn tensor_sqrt(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_sqrt(t(a))) }
    #[inline] fn tensor_sin(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_sin(t(a))) }
    #[inline] fn tensor_cos(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_cos(t(a))) }
    #[inline] fn tensor_tan(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_tan(t(a))) }
    #[inline] fn tensor_tanh(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_tanh(t(a))) }
    #[inline] fn tensor_sigmoid(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_sigmoid(t(a))) }
    #[inline] fn tensor_relu(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_relu(t(a))) }
    #[inline] fn tensor_gelu(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_gelu(t(a))) }
    #[inline] fn tensor_silu(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_silu(t(a))) }

    // ========== Reduction ==========
    #[inline] fn tensor_sum(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_sum(t(a))) }
    #[inline] fn tensor_mean(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_mean(t(a))) }
    #[inline] fn tensor_max(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_max(t(a))) }
    #[inline] fn tensor_min(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_min(t(a))) }
    #[inline] fn tensor_softmax(&self, a: *mut c_void, dim: i64) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_softmax(t(a), dim)) }
    #[inline] fn tensor_max_dim(&self, a: *mut c_void, dim: usize, keep_dim: bool) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_max_dim(t(a), dim, keep_dim)) }
    #[inline] fn tensor_min_dim(&self, a: *mut c_void, dim: usize, keep_dim: bool) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_min_dim(t(a), dim, keep_dim)) }
    #[inline] fn tensor_mean_dim(&self, a: *mut c_void, dim: usize, keep_dim: bool) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_mean_dim(t(a), dim, keep_dim)) }
    #[inline] fn tensor_sum_dim(&self, a: *mut c_void, dim: usize, keep_dim: bool) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_sum_dim(t(a), dim, keep_dim)) }
    #[inline] fn tensor_argmax(&self, a: *mut c_void, dim: i64, keep_dim: bool) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_argmax(t(a), dim, keep_dim)) }
    #[inline] fn tensor_argmin(&self, a: *mut c_void, dim: i64, keep_dim: bool) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_argmin(t(a), dim, keep_dim)) }
    #[inline] fn tensor_tril(&self, a: *mut c_void, diagonal: i64) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_tril(t(a), diagonal)) }
    #[inline] fn tensor_clamp(&self, a: *mut c_void, min: f64, max: f64) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_clamp(t(a), min, max)) }
    #[inline] fn tensor_sample(&self, a: *mut c_void, temp: f32, top_p: f32) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_sample(t(a), temp, top_p)) }

    // ========== Autograd ==========
    #[inline] fn tensor_backward(&self, a: *mut c_void) -> BackendResult<()> { ffi_ops::tl_metal_backward(t(a)); Ok(()) }
    ...
    #[inline] fn tensor_reshape_dims(&self, a: *mut c_void, d1: i64, d2: i64, d3: i64, d4: i64) -> BackendResult<*mut c_void> {
    ...
    #[inline] fn tensor_transpose(&self, a: *mut c_void, dim0: usize, dim1: usize) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_transpose(t(a), dim0, dim1)) }
    #[inline] fn tensor_slice(&self, a: *mut c_void, dim: i64, start: i64, len: i64) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_slice(t(a), dim as usize, start as usize, len as usize)) }
    #[inline] fn tensor_narrow(&self, a: *mut c_void, dim: usize, start: usize, len: usize) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_narrow(t(a), dim, start, len)) }
    #[inline] fn tensor_cat(&self, a: *mut c_void, b: *mut c_void, dim: i64) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_cat(t(a), t(b), dim)) }
    #[inline] fn tensor_cat_i64(&self, a: *mut c_void, b: *mut c_void, dim: i64) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_cat_i64(t(a), t(b), dim)) }
    #[inline] fn tensor_cat2(&self, a: *mut c_void, b: *mut c_void, dim: i64) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_cat(t(a), t(b), dim)) }
    #[inline] fn tensor_cat_4d(&self, a: *mut c_void, b: *mut c_void, dim: i64) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_cat(t(a), t(b), dim)) }
    #[inline] fn tensor_replace_data(&self, dst: *mut c_void, src: *mut c_void) -> BackendResult<()> { ffi_ops::tl_metal_replace_data(t(dst), t(src)); Ok(()) }
    #[inline] fn tensor_repeat_interleave(&self, a: *mut c_void, repeats: usize, dim: usize) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_repeat_interleave(t(a), repeats, dim)) }
    #[inline] fn tensor_to_device(&self, a: *mut c_void, device_id: i32) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_to_device(t(a), device_id)) }
    #[inline] fn tensor_to_f32(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_to_f32(t(a))) }
    #[inline] fn tensor_to_i64(&self, a: *mut c_void) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_to_i64(t(a))) }
    #[inline] fn tensor_embedding(&self, w: *mut c_void, idx: *mut c_void, pad: i64, sg: bool, sp: bool) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_embedding(t(w), t(idx), pad, sg, sp)) }

    // ========== LLM ==========
    #[inline] fn tensor_rms_norm(&self, a: *mut c_void, w: *mut c_void, eps: f32) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_rms_norm(t(a), t(w), eps)) }
    #[inline] fn tensor_rope_new_cos(&self, dim: usize, seq_len: usize, base: f32) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_rope_new_cos(dim, seq_len, base)) }
    #[inline] fn tensor_rope_new_sin(&self, dim: usize, seq_len: usize, base: f32) -> BackendResult<*mut c_void> { v(ffi_ops::tl_metal_rope_new_sin(dim, seq_len, base)) }
    #[inline] fn tensor_apply_rope(&self, a: *mut c_void, cos: *mut c_void, sin: *mut c_void) -> BackendResult<*mut c_void> {
        if a.is_null() || cos.is_null() || sin.is_null() { return Ok(a); }
    ...
    // ========== IO / Print ==========
    #[inline] fn tensor_print(&self, a: *mut c_void) -> BackendResult<()> {
    ...
    #[inline] fn tensor_display(&self, a: *mut c_void) -> BackendResult<()> { self.tensor_print(a) }
    #[inline] fn tensor_print_1(&self, a: *mut c_void) -> BackendResult<()> { self.tensor_print(a) }
    #[inline] fn tensor_print_2(&self, a: *mut c_void) -> BackendResult<()> { self.tensor_print(a) }
    #[inline] fn tensor_print_3(&self, a: *mut c_void) -> BackendResult<()> { self.tensor_print(a) }
    #[inline] fn tensor_save(&self, a: *mut c_void, path: *const i8) -> BackendResult<()> {
        if a.is_null() || path.is_null() { return Ok(()); }
        unsafe {
    ...
    #[inline] fn tensor_load(&self, path: *const i8) -> BackendResult<*mut c_void> {
    ...
    // ========== NN ==========
    #[inline] fn tensor_conv2d(&self, input: *mut c_void, weight: *mut c_void, bias: *mut c_void, stride: usize, padding: usize, dilation: usize, groups: usize) -> BackendResult<*mut c_void> {
    ...
    #[inline] fn tensor_transpose_2d(&self, a: *mut c_void) -> BackendResult<*mut c_void> {
        self.tensor_transpose(a, 0, 1)
    }
    #[inline] fn tensor_reshape_2d(&self, a: *mut c_void, d0: i64, d1: i64) -> BackendResult<*mut c_void> {
    ...
    #[inline] fn tensor_reshape_3d_to_2d(&self, a: *mut c_void, d0: i64, d1: i64) -> BackendResult<*mut c_void> {
        self.tensor_reshape_2d(a, d0, d1)
    }
    #[inline] fn tensor_matmul_4d(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<*mut c_void> {
        self.tensor_matmul(a, b)
    }
    #[inline] fn tensor_add_4d(&self, a: *mut c_void, b: *mut c_void) -> BackendResult<*mut c_void> {
        self.tensor_add(a, b)
    }
    #[inline] fn tensor_silu_4d(&self, a: *mut c_void) -> BackendResult<*mut c_void> {
        self.tensor_silu(a)
    }
}
