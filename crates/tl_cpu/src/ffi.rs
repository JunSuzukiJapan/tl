...
pub extern "C" fn tl_cpu_tensor_from_vec_u8(
    ...
    assert!(rank < 10, "RANK IS HUGE: {}", rank);
    let _ = std::fs::write("DEBUG_RANK.txt", format!("rank={}\n", rank));
    ...
    let _ = std::fs::write("DEBUG.txt", format!("from_vec_u8: rank={}, shape={:?}, numel={}, offset={}\n", rank, shape, numel, offset));
    eprintln!("[CPU_FFI] from_vec_u8: rank={}, shape={:?}, numel={}, offset={}", rank, shape, numel, offset);
...
pub extern "C" fn tl_cpu_tensor_from_vec_u8_f32_shape(
    data: *mut std::ffi::c_void,
    offset: i64,
    shape_ptr: *const f32,
    rank: i64,
...
pub extern "C" fn tl_cpu_tensor_rand(
    rank: i64,
    ...
    let numel: usize = shape.iter().product();
    // Simple LCG-based uniform random [0, 1)
