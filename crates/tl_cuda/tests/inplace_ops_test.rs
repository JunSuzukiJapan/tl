...
fn test_inplace_add_assign() {
    ...
    let result = a.add_impl(&b).unwrap();
    a = result;
...
fn test_inplace_sub_assign() {
    ...
    let result = a.sub_impl(&b).unwrap();
    a = result;
...
fn test_inplace_mul_assign() {
    ...
    let result = a.mul_impl(&b).unwrap();
    a = result;
...
fn test_inplace_div_assign() {
    ...
    let result = a.div_impl(&b).unwrap();
    a = result;
...
fn test_inplace_add_scalar() {
    ...
    let result = a.add_scalar_impl(10.0).unwrap();
    assert_tensor_approx_eq(&result, &[11.0, 12.0, 13.0], 1e-5);

    // 負の値の加算 = 減算
    let result2 = a.add_scalar_impl(-1.0).unwrap();
...
fn test_inplace_sub_scalar() {
    ...
    let result = a.add_scalar_impl(-5.0).unwrap();
...
fn test_inplace_mul_scalar() {
    let a = CudaTensor::from_slice(&[1.0f32, 2.0, 3.0], &[3], DType::F32);
    let result = a.mul_scalar_impl(3.0).unwrap();
...
fn test_inplace_div_scalar() {
    let a = CudaTensor::from_slice(&[10.0f32, 20.0, 30.0], &[3], DType::F32);
    let result = a.div_scalar_impl(10.0).unwrap();
...
fn test_clone_data() {
    ...
    let a_modified = a.add_scalar_impl(10.0).unwrap();
...
fn test_multiple_inplace_updates() {
    ...
    for _ in 0..10 {
        let result = a.add_impl(&increment).unwrap();
        a = result;
    }
...
fn test_mul_scalar_zero() {
    let a = CudaTensor::from_slice(&[1.0f32, 2.0, 3.0], &[3], DType::F32);
    let result = a.mul_scalar_impl(0.0).unwrap();
...
fn test_add_scalar_zero() {
    let a = CudaTensor::from_slice(&[1.0f32, 2.0, 3.0], &[3], DType::F32);
    let result = a.add_scalar_impl(0.0).unwrap();
    assert_tensor_approx_eq(&result, &[1.0, 2.0, 3.0], 1e-5);
}
