...
use crate::quantized::{GGMLType, QTensor};
...
use gguf_rs::{ByteOrder, GGMLType as RsGgmlType, GGUFContainer};
...
pub extern "C" fn tl_gguf_load(path: *mut StringStruct) -> *mut OpaqueTensorMap {
    unsafe {
        ...
        eprintln!(
            "[tl_gguf_load] Meta end: {}, Align: {}, Data offset: {}",
            metadata_end, alignment, data_offset
        );
        ...
        for tensor in model.tensors() {
            ...
            if ggml_type == GGMLType::Unknown {
                eprintln!(
                    "[tl_gguf_load] Skipping tensor '{}': unsupported type kind={} ({:?})",
                    name, tensor.kind, rs_type
                );
                continue;
            }
        ...
        Box::into_raw(Box::new(OpaqueTensorMap { entries, qtensors }))
    }
}
